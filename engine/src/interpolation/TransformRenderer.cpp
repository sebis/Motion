...
namespace Interpolation
{
	...
	void TransformRenderer::draw()
	{
		...
		m_shader->setUniform("world", m_gameObject->transform().world() * glm::rotate(mx, 90.0f, glm::vec3(0, 1, 0)));
		m_pitch->draw();		

		// mesh is oriented to Z-axis so rotate for correct axis
		m_shader->setUniform("world", m_gameObject->transform().world() * glm::rotate(mx * my, 90.0f, glm::vec3(1, 0, 0)));
		m_yaw->draw();

		m_shader->setUniform("world", m_gameObject->transform().world() * mx * my * mz);
		m_roll->draw();
